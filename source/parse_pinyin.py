...
from source.mandarin_utils import syllables_list, diacritics_to_base_letter, single_letter_consonant, vowel, diacritics_to_decimals

def clean_pinyin_text(text: str) -> str:
    """
...
def is_valid_pinyin_string(pinyin_str:str) -> bool:
    """
...
def remove_pinyin_diacritics(pinyin_str:str) -> str:
    """
...
def pinyin_diacritics_to_decimals(syllable:str) -> str:
    """
    ...
    for char in syllable:
       if char in diacritics_to_decimals:
          output = syllable.replace(char,diacritics_to_decimals[char][0])
          output += str(diacritics_to_decimals[char][1])
          break
    return output

def remove_pinyin_decimals():
    pass

def tokenize_pinyin_text(text:str) -> list[str]:
    """
    ...
    output = []
    eval_str = ""
    text = clean_pinyin_text(text)
    text += " " if text[-1] != " " else "" #Needed to return end of string - bit hacky...
    for char in text.lower():
        ...
        if len(eval_str) > 3:
            temp_state = is_valid_pinyin_string(eval_str)
            temp_string = remove_pinyin_diacritics(eval_str)

            #Check if this is a valid pinyin string
            if is_valid_pinyin_string(remove_pinyin_diacritics(eval_str)) == False:
                if eval_str[-2] in single_letter_consonant:
                    if remove_pinyin_diacritics(eval_str)[-1] in vowel:
